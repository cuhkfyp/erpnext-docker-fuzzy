...
def materialize_identity(
    *,
    ...
    if preview["already_applied"]:
        from db_connector.api_unified_person import reconcile_unified_person_scope

        unified_result = reconcile_unified_person_scope(
            plan.record_ids,
            reason="identity_materialization_idempotent_reconciliation",
            origin_doctype=origin_doctype,
            origin_document=origin_document,
        )
        return {
            ...
            "unified_person_reconciliation": unified_result,
        }
    if not preview["safe"]:
    ...
    unified_result = reconcile_unified_person_scope(
        plan.record_ids,
        reason="identity_materialization_reconciliation",
        origin_doctype=origin_doctype,
        origin_document=origin_document,
    )
...
def get_identity_resolution(ccd_master_name: str) -> dict[str, Any]:
    ...
    from db_connector.api_unified_person import unified_person_payload_for_record

    unified_person = unified_person_payload_for_record(ccd_master_name)
...
def end_membership(
    ...
    affected_records.add(str(membership.ccd_master))
    ...
    reconcile_unified_person_scope(
        affected_records,
        reason="identity_membership_ended",
        origin_doctype=MEMBERSHIP_DOCTYPE,
        origin_document=membership.name,
    )
    frappe.db.commit()
