...
# Restore the host-persistent CCD Portal after container recreation. This is a
# no-op until deploy_ccd_portal.sh has been installed by its reviewed deploy.
PORTAL_DEPLOY_SCRIPT="$ROOT_DIR/deploy_ccd_portal.sh"
if [[ -x "$PORTAL_DEPLOY_SCRIPT" ]]; then
	"$PORTAL_DEPLOY_SCRIPT" --runtime-only --restart
fi
