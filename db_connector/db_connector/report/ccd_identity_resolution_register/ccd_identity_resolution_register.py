...
def _normalized_filters(
    ...
    if not reveal_identifiers and any(
        ...
        for fieldname in (
            "ccd_master", "ccd_reg_source", "identity_group", "unified_person"
        )
    ):
        frappe.throw(
...
def _current_resolution_rows() -> list[dict[str, Any]]:
    ...
    unified_people = _unified_people_for_records(candidate_ids)
            ...
            output.append(
                {
                    ...
                    "unified_person": unified_people.get(record_id, ""),
...
def _unified_people_for_records(record_ids: Iterable[str]) -> dict[str, str]:
    if not frappe.db.table_exists("CCD Unified Person Membership"):
        return {}
    output: dict[str, str] = {}
    ...
    for offset in range(0, len(ordered), 500):
        for row in frappe.get_all(
            ...
            filters={
                "ccd_master": ["in", ordered[offset : offset + 500]],
                "status": "Active",
            },
            fields=["ccd_master", "unified_person"],
            limit_page_length=500,
        ):
            output[str(row.ccd_master)] = str(row.unified_person)
    return output
...
def _apply_filters(
    ...
    for row in rows:
        ...
        if filters.get("ccd_master") and row["ccd_master"] != filters.ccd_master:
            continue
        if filters.get("unified_person") and row["unified_person"] != filters.unified_person:
            continue
...
def _masked_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        person = str(row.get("unified_person") or "")
        if person and person not in person_aliases:
            person_aliases[person] = f"Masked Unified Person {len(person_aliases) + 1}"
        masked.update(
            {
                "ccd_master": f"Masked Record {index}",
                "unified_person": person_aliases.get(person, ""),
...
def _columns(reveal_identifiers: bool) -> list[dict[str, Any]]:
    return [
        {
            "fieldname": "unified_person",
            "label": _("Unified Person Number") if reveal_identifiers else _("Masked Unified Person"),
            "fieldtype": "Link" if reveal_identifiers else "Data",
            **({"options": "CCD Unified Person"} if reveal_identifiers else {}),
            "width": 175,
        },
        {
