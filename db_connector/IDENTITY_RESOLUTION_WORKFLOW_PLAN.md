...
| Item | Value |
| --- | --- |
| Status | Phases 1–7 implemented; identity writes and both automatic controls remain disabled unless separately authorized |
...
| Integrity-restoration update | Clean replacement generation accepted; Phase 7 permanent Unified Person registry and verified backfill completed on 2026-09-16 while materialization and automatic controls remained disabled |
...
### Phase 7 — permanent Unified Person numbers

Completed on the development site on 2026-09-16 after a fresh verified full
backup:

1. create a durable registry, current/historical Memberships, alias lineage,
   immutable lifecycle Events, and restartable Backfill Runs;
2. issue `HKSR-U` plus nine non-recycled sequence digits and one Luhn check
   digit without adding or updating a field on CCD Master;
3. backfill every current singleton and governed Identity Group in bounded,
   committed batches, with an unchanged CCD Master count/`modified` hash as an
   acceptance gate;
4. make the oldest number survive a merge, retain other numbers as resolvable
   aliases, restore unambiguous original numbers after a split, and issue a new
   number when restoration is not safe;
5. end assignments inside the governed source-retirement transaction and
   restore the same canonical number after technical delete/recreate only for
   an exact, unique, inactive stable source lineage;
6. issue new CCD Masters as singleton people in the insert transaction, with a
   bounded daily reconciliation/audit for missed raw-import hooks; and
7. expose current numbers through the CCD Master Identity Resolution tab,
   sensitive register/API, masked operational register, and pseudonymous
   dashboard anchor.

The completed live backfill is `8srqf32s7o`: 256,046 permanent people and
256,095 active Memberships, zero integrity issues, and an identical before/
after CCD Master `modified` snapshot.
...
five completed Same reviews. Step 9 was completed as Phase 7 after its separate
verified backup; it did not authorize wider Tiered or Splink materialization. Current
...
### 27.4 Permanent Unified Person completion — 2026-09-16

Fresh full backup `20260916_154024-frontend-*` passed database, site-config,
public-files, and private-files integrity and SHA-256 validation before Phase 7
code or data writes. Materialization, Automatic Tiered, and Automatic QC were
all off.

Restartable Backfill Run `8srqf32s7o` completed 52 bounded transactions. It
issued 256,046 unique non-recycled numbers and assigned all 256,095 current CCD
Masters. All integrity checks are zero and the CCD Master count and `modified`
snapshot SHA-256 are unchanged. Exact stable source lineage supports recovery
of the same number after a governed technical delete/recreate. Ambiguous,
duplicate, or incomplete lineage never guesses; it receives a separate
singleton and an operational error for review.

Future governed Same decisions reconcile the registry in the same transaction.
The oldest number remains canonical after merge and every superseded number
remains a resolvable alias. Governed Different/correction paths can split the
