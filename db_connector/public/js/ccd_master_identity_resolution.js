...
function unified_person_html(payload) {
	if (!payload.unified_person_number) {
		return `<p><strong>${__("Unified Person Number")}</strong>: ${__("Pending initial assignment")}</p>`;
	}
	const number = identity_esc(payload.unified_person_number);
	const value = payload.sensitive_values_visible
		? `<a href="/app/ccd-unified-person/${encodeURIComponent(payload.unified_person_number)}">${number}</a>`
		: number;
	const aliases = (payload.unified_person_aliases || []).length
		? ` <span class="text-muted">(${__("aliases")}: ${(payload.unified_person_aliases || []).map(identity_esc).join(", ")})</span>`
		: "";
	return `<p><strong>${__("Unified Person Number")}</strong>: ${value}${aliases}</p>`;
}
