...
recommendation register. `api_fuzzy_review_queue.py` uses two dedicated
DocTypes and the existing immutable label child table:

- `CCD Match Review Queue Run` freezes the canary, policy snapshot, approved
  threshold evaluation, adapter version, maximum-F1 cutoff, and aggregate
  generation/review counts.
- `CCD Match Review Candidate` stores one versioned model-`Review` pair, its
  priority rank, protected evidence context, staleness state, and human final
  label. The individual score is restricted to System Managers.
- `CCD Match Review Label` preserves ordinary and adjudication submissions.

For this queue, *eligible* means a pair generated by the complete governed
blocking policy after excluding:

1. all Tiered High pairs from the selected canary, including both `Proposed`
   and `Exception`; and
...
automatic High threshold, and a score below it is not a `Different` decision.
Ordinary reviewers see masked identity values and no CCD record IDs. Sensitive
Reviewers retain full permitted values and record links, while probability and
blocking diagnostics remain System Manager-only.

Humans label `Same`, `Different`, or `Unsure`. A positive result requires two
independent `Same` submissions. `Unsure` and disagreement route to manager
adjudication; a positive adjudication still needs independent confirmation.
The human final label never rewrites the stored model tier `Review`. Queue
...
After a matching canary is `Ready` or `Active`, create its optional Splink
priority queue through the manager-protected Desk button or:
...
Only one running or ready queue is allowed per canary. The queue form links to
its `CCD Match Review Candidate` list, already ordered by priority rank.
